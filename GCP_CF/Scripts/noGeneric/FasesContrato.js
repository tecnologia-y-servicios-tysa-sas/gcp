            ...
            if (data.fases && data.fases.length > 0) {       
                ...
                $.each(data.fases, function (key, entry) {
                    ...
                    if (entry.Actividades && entry.Actividades.length > 0) {
                        tblRow += ProcesarActividadesFase(entry.Actividades, idContrato, entry.Id)
                    } else {
...
function ProcesarActividadesFase(actividades, idContrato, idFase) {
    ...
    for (var i = 1; i <= actividades.length; i++) {
        ...
        tblActividades += "<tr id=\"rowActividadFase_C" + idContrato + "_F" + idFase + "_A" + ac.Id + "\">"
            ...
            + "<td class=\"col-md-1 col-sm-1 col-xs-1\" style=\"padding: 4px\">"
        ...
        if (confirm(mensajeConfirmacion)) {
            ...
            $.ajax({
                ...
                data: JSON.stringify({
                    ...
                    fechaFin: fechaFin, estadoActividad: estadoActividad
                }),
                dataType: 'json',
                contentType: "application/json; charset=utf-8",
                beforeSend: function () {
